...
def run_benchmark(file_index, noise_level, data_augmentation, n):
    seed = 10 * file_index
    np.random.seed(seed)
    ...
    x_train = np.concatenate((x_clean, x_train), axis=0)
    y_train = np.concatenate((y_clean, y_train), axis=0)
    ...
    if not data_augmentation:
        dirs = 'record_new_preprocessing/ResNet' + str(depth) + '_pre-activation_benchmark/noise_' + str(noise_level) + '_test' + str(file_index) + '_lr_' + str(lr_init)
    else:
        dirs = 'record_new_preprocessing/ResNet' + str(depth) + '_pre-activation_benchmark_data_augmentation/noise_' + str(noise_level) + '_test' + str(file_index) + '_lr_' + str(lr_init)
...
#     callbacks = [checkpoint, lr_reducer, lr_scheduler]
    ...
    if not data_augmentation:
        ...
        History_benchmark = model.fit(x_train, y_train,
        ...
        History_benchmark = model.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size),
...
for noise_level in [0]:
    for n in [2]:
        ...
        sess = tf.Session(config=config)
        K.set_session(sess)
        run_benchmark(file_index, noise_level, data_augmentation, n)
