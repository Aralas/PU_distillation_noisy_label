...
from generate_ResNet import ResNet
...
path_name = '/benchmark_ResNet%d_pre-activation_lr_%.4f' % (depth, learning_rate)
...
y_train = np.concatenate((y_train, y_clean), axis=0)

# Generate model.
model_object = ResNet(10, input_shape, learning_rate, n)
...
lr_scheduler = LearningRateScheduler(model_object.lr_schedule)
...
if not data_augmentation:
    ...
    History = model_object.model.fit(x_train, y_train,
    ...
    History = model_object.model.fit_generator(datagen.flow(x_train, y_train, batch_size=batch_size),
...
# Score trained model.
model_object.model.load_weights(filepath)
scores = model_object.model.evaluate(x_test, y_test, verbose=1)
