...
def run_benchmark(file_index, noise_level, learning_rate, seed, architecture):
    np.random.seed(seed)
    ...
    # create record files
    dirs = 'record_new_preprocessing/benchmark_test_architecture/noise_' + str(noise_level) + '_lr_' + str(learning_rate) + '/' + str(architecture)
...
for file_index in range(5):
    for noise_level in [0, 0.5, 0.8]:
        for learning_rate in [0.0001, 0.0003, 0.001]:
            ...
            run_benchmark(file_index, noise_level, learning_rate, seed, architecture)
