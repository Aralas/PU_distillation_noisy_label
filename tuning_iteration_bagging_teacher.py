...
for label in range(10):
    ...
    for line in lines:
        new_line += line.strip('\n').replace('array','')
    final_additional_data[label] = eval(new_line)[iteration_k]
    add_number.append(len(eval(new_line)[iteration_k]))
...
for k in range(2): 
    ...
    for lambda_teacher in [0.3, 0.4, 0.5, 0.6, 0.7, 0.8]:
        ...
        file_student  = open(dirs + 'iteration_' + str(iteration_k) + '/bagging_teacher_file_student_' + str(k) + '.txt', 'a+')
