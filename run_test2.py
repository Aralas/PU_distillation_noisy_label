...
path_name = '/positive_threshold_%.2f_add_criterion_%d_learning_rate_%.4f' % (
...
record_file.write(
    ...
    add_criterion:%d\n K_iteration:%d\n N_bagging:%d\n batch_size:%d\n epochs:%d\n learning_rate:%f\n '
    'data_augmentation:%s\n' % (seed, noise_level, clean_data_size, additional_data_limitation[0], \
                                additional_data_limitation[1],positive_threshold, add_criterion, K_iteration, \
                                N_bagging, batch_size, epochs, learning_rate, str(data_augmentation)))
record_file.close()


def generate_model(num_classes, input_shape, learning_rate):
    model = Sequential()
...
for i in label_list:
    ...
    for n in range(N_bagging):
        if n % 10 == 9:
            clear_session()
        model = generate_model(1, input_shape, learning_rate)
