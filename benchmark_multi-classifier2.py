...
def run_benchmark(file_index, noise_level, learning_rate, seed):
    ...
    dirs = 'record_new_preprocessing/benchmark/noise_' + str(noise_level) + '_lr_' + str(learning_rate) + '_clean_data_size_' + str(clean_data_size) + '/' + str(architecture) + '/'
    ...
    History_benchmark = benchmark_model.fit(x, y, validation_data=(x_test, y_test), batch_size=64, epochs=50,
                                          shuffle=True)
