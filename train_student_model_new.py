...
path_name = '/student_model_partial_data'
...
index_moderate = np.where((y_max >= 0.5) & (0.8 > y_max))[0]
index_low = np.where(0.5 > y_max)[0]

x = np.concatenate((x_clean, x_train[index_high], x_train[index_moderate]), axis=0)
print(len(index_high), len(index_moderate), len(index_low), len(x))
if label_type == 'hard':
    y_high = tf.contrib.keras.utils.to_categorical(np.argmax(y_pred[index_high], axis=1), 10)
    y_moderate = tf.contrib.keras.utils.to_categorical(np.argmax(y_pred[index_moderate], axis=1), 10) + y_train[index_moderate] * 0.5
elif label_type == 'soft':
    y_high = y_pred[index_high]
    y_moderate = y_pred[index_moderate] * 0.5 + y_train[index_moderate] * 0.5

y = np.concatenate((y_clean, y_high, y_moderate), axis=0)
...
# model_name = 'cifar10_%s_noise_%.1f_threshold_%.1f_lambda_%.1f_file%d.h5' % (label_type, noise_level, student_threshold, student_lambda, file_index)
model_name = 'cifar10_%s_noise_%.1f_threshold_%.1f__file%d.h5' % (label_type, noise_level, student_threshold, file_index)
...
# Record accuracy.
# record_file = open(os.path.join(precision_dir, 'noise_%.1f_%s_threshold_%.1f_lambda_%.1f_file%d.txt' % (noise_level, label_type, student_threshold, student_lambda, file_index)), 'a+')
record_file = open(os.path.join(precision_dir, 'noise_%.1f_%s_threshold_%.1f_file%d.txt' % (noise_level, label_type, student_threshold, file_index)), 'a+')
