...
def main(lr, index, mixup=False):
    ...
    class_weight = [1/x for x in add_number]
    class_weight = class_weight / np.sum(class_weight)
    class_weight = torch.FloatTensor(class_weight)
    class_weight = class_weight.to(device)

    # Generate indices of augmented subset
    train_index = clean_index
    y_slice = sum([[i]*clean_data_size for i in range(10)], [])
    for label in range(10):
        indices = add_index[label]
        train_index = np.concatenate([train_index, indices])
        y_slice += [label]*len(indices)

    augmented_train_set = Subset(train_set, train_index+len(y_valid))
    train_loader = torch.utils.data.DataLoader(
        dataset=augmented_train_set,
    ...
    if mixup:
        path = "record/teacher_mixup_weighted_loss_new2/"
        model_path = "saved_model/teacher_weighted_mixup_new2/"
    else:
        path = "record/teacher_weighted_loss_new2/"
        model_path = "saved_model/teacher_weighted_new2/"
