...
def main(lr, noise, index, student_lambda, student_threshold):
    seed = 99
    np.random.seed(seed)
    ...
    path = "record/PreAct_student_update_high_confident_data/noise"+str(noise)+"/"
    if not os.path.exists(path):
        os.makedirs(path)
    f = open(path + "acc_lr_%.3f_lambda_%.2f_threshold_%.2f_%d.txt" % (lr, student_lambda, student_threshold, index), "a+")
    ...
    for epoch in range(epoch_nums):
        ...
        for inputs, labels in train_loader:
            ...
            with torch.no_grad():      
                ...
                y_pseudo = labels.numpy().reshape(-1)
                y_pseudo = np.eye(10)[y_pseudo]   # one hot
...
student_lambda = 0.05
for student_threshold in [0]:
    for lr in [0.05]:
        for index in range(1):
            for noise in [0.7, 0.9]:
                main(lr, noise, index, student_lambda, student_threshold)
