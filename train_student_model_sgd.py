...
y_pseudo = student_lambda * y_train + (1 - student_lambda) * y_pred
...
model_name = 'cifar10_SGD_noise_%.1f_lambda_%.2f_file%d.h5' % (noise_level, student_lambda, file_index)
...
if not data_augmentation:
    ...
    record_file = open(os.path.join(precision_dir, 'SGD_noise_%.1f_lambda_%.2f_accuracy_file%d.txt' % (noise_level, student_lambda, file_index)), 'a+')
    record_file.write('training, validation, test\n')
    for epoch in range(epochs):
        ...
        record_file.write(str(History.history['acc']) + ',' + str(History.history['val_acc']) + ',' + str(scores[1]) + '\n')
        record_file.flush()
