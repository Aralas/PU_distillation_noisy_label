...
"""
if not bagging:
    teacher_path_name = '/teacher_model'
else:
    teacher_path_name = '/teacher_model_bagging'
...
y_teacher = np.mean(y_pred, axis=0)

"""
# Generate model.
"""
model = resnet_v2(input_shape, depth, num_classes)
model_name = 'cifar10_noise_%.1f_lambda_%.2f_file%d.h5' % (noise_level, lambda_t, file_index)
...
if not data_augmentation:
    ...
    for epoch in range(epochs):
        if epoch < 10:
            ...
            lambda_s = val_acc[-1] * 0.5

        # y_pseudo = lambda_t * y_train + (1 - lambda_t) * y_teacher
        lambda_t_ = min(lambda_t, 1 - lambda_s)
        y_student = model.predict(x_train)
        y_pseudo = lambda_t_ * y_teacher + lambda_s * y_student + (1 - lambda_t_ - lambda_s) * y_train
        y = np.concatenate((y_pseudo, y_clean), axis=0)

        History = model.fit_generator(datagen.flow(x, y, batch_size=batch_size),
                                      steps_per_epoch=int(x_train.shape[0] / 10),
                                      validation_data=(x_validation, y_validation),
...
# Record accuracy.
record_file = open(
    os.path.join(precision_dir, 'noise_%.1f_lambda_%.2f_accuracy_file%d.txt' % (noise_level, lambda_t, file_index)),
    'a+')
